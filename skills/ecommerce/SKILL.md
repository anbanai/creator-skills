...
- 生成 `output/manifest.json`（按模块：文件名/尺寸/用途/provider/自检结果/合规状态）
- 直接校验 manifest 中的文件与所有最终产物对应的显式 `output/<filename>` 路径
- 报告交付路径 `output/` + `submit_agent_feedback(...)`
