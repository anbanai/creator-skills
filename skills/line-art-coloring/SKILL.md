...
| `download_image` (project_id, url, upload) | 下载在线图片到 MCP 服务器临时路径或上传到存储，返回服务器端 `file_path`；**不写入 agent 本地 `output`** |
...
- 记录实际 prompt + provider + model 到 `output/image-prompts.md`，便于复盘 504、色偏和线稿偏移
...
`output_path` 是 MCP 服务器端路径，不是客户端当前目录路径。固定使用 `/tmp/anban-creator-line-art/$TASK_ID/colored_NN_a.png`，返回的 `file_path` 写入 `output/server-paths.md`。`size` 从原始线稿推断最接近的支持比例（如 7:5 接近 `3:2` 或 `4:3`），传入 `size="3:2"`；返回后用文件尺寸或 `analyze_image` 检查是否被裁切、变形或转为竖图。
...
# 把 prompt/provider/model/size/output_path/ref_image_path/revised_prompt 追加 output/image-prompts.md
```
